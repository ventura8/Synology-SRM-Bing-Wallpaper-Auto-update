...
- Keep workflow-level `permissions: contents: read`; add write scopes per job.
- The `sonarqube` job uses `SONAR_TOKEN` and `sonar-project.properties`; keep it
  skipping cleanly when the secret is unavailable.
