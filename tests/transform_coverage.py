...
def generate_badge(line_rate, output_path="assets/coverage.svg"):
    ...
    label_len = label_width * 10 - 100
    value_len = value_width * 10 - 100
    ...
    svg = (
        ...
        f'        <text aria-hidden="true" x="{int(label_x)}" y="150" fill="#010101" '
        f'fill-opacity=".3" transform="scale(.1)" textLength="{label_len}">{label_text}</text>\n'
        f'        <text x="{int(label_x)}" y="140" transform="scale(.1)" fill="#fff" '
        f'textLength="{label_len}">{label_text}</text>\n'
        f'        <text aria-hidden="true" x="{int(value_x)}" y="150" fill="#010101" '
        f'fill-opacity=".3" transform="scale(.1)" textLength="{value_len}">{value_text}</text>\n'
        f'        <text x="{int(value_x)}" y="140" transform="scale(.1)" fill="#fff" '
        f'textLength="{value_len}">{value_text}</text>\n'
        "    </g>\n"
        "</svg>"
    )
