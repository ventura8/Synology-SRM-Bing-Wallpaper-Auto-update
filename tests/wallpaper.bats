...
@test "Script should archive wallpaper if enabled" {
    ...
    original_archive_setting="$(grep '^ENABLE_ARCHIVE=' "$SCRIPT")"
    restore_archive_setting() {
        sed -i "s/^ENABLE_ARCHIVE=.*/${original_archive_setting}/" "$SCRIPT"
    }
    trap restore_archive_setting RETURN

    sed -i 's/^ENABLE_ARCHIVE=.*/ENABLE_ARCHIVE=true/' "$SCRIPT"
...
@test "Script should handle font download failure (Skip Overlay)" {
    ...
    # Simulate missing ImageMagick by overriding `which convert` via PATH-local shim.
    mkdir -p /tmp/bin_no_convert
    cat <<EOT >/tmp/bin_no_convert/which
#!/bin/bash
if [ "\$1" = "convert" ]; then
    exit 1
fi
/usr/bin/which "\$@"
EOT
    chmod +x /tmp/bin_no_convert/which

    ORIG_PATH="$PATH"
    cleanup_no_convert_mock() {
        export PATH="$ORIG_PATH"
        rm -rf /tmp/bin_no_convert
    }
    trap cleanup_no_convert_mock RETURN

    export PATH="/tmp/bin_no_convert:$PATH"
